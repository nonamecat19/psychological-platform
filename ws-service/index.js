...
io.on("connection", (socket) => {
    ...
    socket.on("SEND_MESSAGE_GROUP", async (data) => {
        ...
        try {
            ...
            const client = await pool.connect();
            try {
                const result = await client.query(`INSERT INTO messages (therapy_group_id, content, user_id)
                                                   VALUES ($1, $2, $3) RETURNING *`, [chatId, message, decoded.ID]);
                const {id, created_at, updated_at, deleted_at, user_id, content, therapy_group_id} = result.rows[0]
                const normalizedMessage = {
                    ID: +id,
                    CreatedAt: created_at,
                    UpdatedAt: updated_at,
                    ...
                    UserID: +user_id,
                    Content: content,
                    TherapyGroupId: therapy_group_id
                }
                io.sockets.emit(`SEND_MESSAGE_GROUP:${chatId}`, JSON.stringify(normalizedMessage));
            } finally {
                client.release();
            }
        } catch (err) {
